...
public class IdAuthServiceImpl implements IdAuthService {
    ...
    public boolean validateOtp(String transactionID, String individualId, String individualIdType, String otp) {
        ...
        } catch (ApisResourceAccessException | InvalidKeySpecException | NoSuchAlgorithmException | IOException | JsonProcessingException e) {
            logger.error(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.USERID.toString(), null,
                    "IdAuthServiceImpl::validateOtp():: validate otp method call"
                            + e.getStackTrace());
        }
        if (response.getErrors() != null && response.getErrors().size() > 0) {
            response.getErrors().stream().forEach(error -> logger.error(
                    LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.USERID.toString(), error.getErrorCode(), error.getErrorMessage()));
    ...
    public AuthResponseDTO internelOtpAuth(String transactionID, String individualId, String individualIdType, String otp)
    ...
    private byte[] encryptRSA(final byte[] sessionKey, String refId, ObjectMapper mapper)
        ...
        try {
            responseWrapper = (ResponseWrapper<?>) restClient.getApi(uriComponent.toUri(), ResponseWrapper.class, tokenGenerator.getToken());
        } catch (Exception e) {
