...
public class AuthFacadeImplTest {
	...
	public void authenticateIndividualTest() throws IdAuthenticationBusinessException, IdAuthenticationDaoException,
		...
		authRequestDTO.setMetadata(Collections.singletonMap(AuthTransactionBuilder.class.getSimpleName(), AuthTransactionBuilder.newInstance()));
		...
		Mockito.when(idService.processIdType(Mockito.any(), Mockito.any(), Mockito.anyBoolean(), Mockito.anyBoolean(), Mockito.anySet())).thenReturn(idRepo);
...
//	}
	...
	public void testProcessPinDetails_pinValidationStatusNull() throws IdAuthenticationBusinessException {
		...
		idInfo.put("email", list);
		idInfo.put("phone", list);
		Mockito.when(idService.processIdType(IdType.UIN.getType(), uin, false, true, Collections.emptySet())).thenReturn(idRepo);
		Mockito.when(idService.getIdByUin(Mockito.anyString(), Mockito.anyBoolean(), Mockito.anySet())).thenReturn(repoDetails());
