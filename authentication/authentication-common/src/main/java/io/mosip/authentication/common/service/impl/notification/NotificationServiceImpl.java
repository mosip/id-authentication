...
public class NotificationServiceImpl implements NotificationService {
    ...
    @Value("${mosip.ida.otp.notification.request.mode:0}")  // 0 sync 1 async
    ...
    public void sendAuthNotification(AuthRequestDTO authRequestDTO, String idvid, AuthResponseDTO authResponseDTO, Map<String, List<IdentityInfoDTO>> idInfo, boolean isAuth) throws IdAuthenticationBusinessException {
        ...
        String authTypeStr = Stream.of(Stream.<AuthType>of(DemoAuthType.values()), Stream.<AuthType>of(BioAuthType.values()), Stream.<AuthType>of(PinAuthType.values()), Stream.<AuthType>of(PasswordAuthType.values()), Stream.<AuthType>of(KeyBindedTokenAuthType.values())).flatMap(Function.identity()).filter(authType -> authType.isAuthTypeEnabled(authRequestDTO, idInfoFetcher)).peek(System.out::println).map(authType -> authType.getDisplayName(authRequestDTO, idInfoFetcher)).distinct().collect(Collectors.joining(","));
    ...
    public void sendOTPNotification(String idvid, String idvidType, Map<String, String> valueMap, List<String> templateLanguages, String otp, String notificationProperty, LocalDateTime otpGenerationTime) throws IdAuthenticationBusinessException {
        ...
        this.sendNotification(otpTemplateValues, valueMap.get(IdAuthCommonConstants.EMAIL), valueMap.get(IdAuthCommonConstants.PHONE_NUMBER), SenderType.OTP, notificationProperty, templateLanguages);
    }
     ...
     *
     */
    private Map<String, Object> getOtpTemplateValues(String idvid, String idvidType, Map<String, String> valueMap, LocalDateTime otpGenerationTime) {
     ...
     *
     * @param timestamp the request time
    ...
    public void sendNotification(Map<String, Object> values, String emailId, String phoneNumber, SenderType sender, String notificationProperty, List<String> templateLanguages) throws IdAuthenticationBusinessException {
        ...
        if (isNotNullOrEmpty(notificationProperty) && !notificationProperty.equalsIgnoreCase(NotificationType.NONE.getName())) {
     ...
     */
    private void processNotification(String emailId, String phoneNumber, Set<NotificationType> notificationType, String notificationTypeConfig) {
     ...
     */
    private String applyTemplate(Map<String, Object> values, String templateName, List<String> templateLanguages) throws IdAuthenticationBusinessException {
        try {
     ...
     *                                           exception
     */
    private void invokeSmsNotification(Map<String, Object> values, SenderType sender, String notificationMobileNo, List<String> templateLanguages) throws IdAuthenticationBusinessException {
        ...
        if (requestMode == 0) {
            notificationManager.sendSmsNotificationAsync(notificationMobileNo, smsTemplate).block();
        } else {
     ...
     *                                           exception
     */
    private void invokeEmailNotification(Map<String, Object> values, String emailId, SenderType sender, List<String> templateLanguages) throws IdAuthenticationBusinessException {
     ...
     * @throws IdAuthenticationBusinessException
     */
    private List<String> getTemplateLanguages(Map<String, List<IdentityInfoDTO>> idInfo) throws IdAuthenticationBusinessException {
        List<String> userPreferredLangs = idInfoFetcher.getUserPreferredLanguages(idInfo);
        List<String> defaultTemplateLanguges = userPreferredLangs.isEmpty() ? idInfoFetcher.getTemplatesDefaultLanguageCodes() : userPreferredLangs;
