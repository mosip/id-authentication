...
public class MatchInputBuilder {
	 ...
	 *
	 * <p>This method uses a {@link LinkedHashSet} to ensure that only unique
	 * MatchInput elements are retained and the order of their first occurrence
	 * is maintained.</p>
	 *
	 * @param inputs the list of MatchInput objects that may contain duplicates
	 * @return a new list containing only unique MatchInput objects in their original order
	 */
	private List<MatchInput> removeDuplicates(List<MatchInput> inputs) {
		// Using LinkedHashSet to preserve order and ensure uniqueness
