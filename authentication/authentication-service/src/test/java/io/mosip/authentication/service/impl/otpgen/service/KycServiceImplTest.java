...
public class KycServiceImplTest {
	...
	public void validUIN7() throws IdAuthenticationDaoException, IOException, IdAuthenticationBusinessException {
		...
		environment.setProperty("ekyc.type.fullkyc", "fullName,firstName,middleName,lastName,dateOfBirth,gender,phone,email,addressLine1,addressLine2,addressLine3,city,province,region,postalCode,face");
		ReflectionTestUtils.setField(kycServiceImpl, "env", environment);
		kycServiceImpl.retrieveKycInfo("12232323121", KycType.FULL, true, true, idInfo);
	}
	
	@Test
	public void validUIN8() throws IdAuthenticationDaoException, IOException, IdAuthenticationBusinessException, NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		MockEnvironment environment = new MockEnvironment();
		ReflectionTestUtils.setField(kycServiceImpl, "env", environment);
						...
						if (val instanceof List) {
							...
							return arrayList.stream().filter(elem -> elem instanceof Map)
									.map(elem -> (Map<String, Object>) elem).map(map1 -> {
										...
										idInfo.setLanguage(map1.get("language")!= null? String.valueOf(map1.get("language")) : null);
