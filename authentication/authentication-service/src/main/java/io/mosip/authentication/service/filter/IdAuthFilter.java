...
public class IdAuthFilter extends BaseAuthFilter {
	...
	protected Map<String, Object> decipherRequest(Map<String, Object> requestBody) throws IdAuthenticationAppException {
		try {
			/*
			 * String partnerId=(String)requestBody.get(PATRNER_ID); String
			 * licenseKey=(String)requestBody.get(MISP_LK); String mispId=
			 * licenseKeyMISPMapping(licenseKey); validPartnerId(partnerId); String
			 * policyId=validMISPPartnerMapping(partnerId, mispId);
			 */
	...
	public void checkAllowedAuthTypebasedOnPolicy(String policyId, AuthRequestDTO authRequestDTO)
		...
		try {
			...
			List<String> allowedauthType = authPolicy.stream().filter(s -> s.isMandatory()).map(s -> s.getAuthType())
					.collect(Collectors.toList());
			List<String> allowedsubAuthType = authPolicy.stream().filter(s -> s.isMandatory()).map(s -> s.getAuthType())
					.collect(Collectors.toList());
			...
			if (authType.isBio() && allowedauthType.contains(MatchType.Category.BIO.name())) {
				List<String> bioInfoList = authRequestDTO.getBioMetadata().stream().map(s -> s.getBioType())
						.collect(Collectors.toList());
	...
	/*
	 * protected String[] getAuthPart(ResettableStreamHttpServletRequest
	 * requestWrapper) { String[] ver = null; if (requestWrapper instanceof
	 * HttpServletRequestWrapper) { String url =
	 * requestWrapper.getRequestURL().toString(); String contextPath =
	 * requestWrapper.getContextPath();
	 * 
	 * if ((Objects.nonNull(url) && !url.isEmpty()) && (Objects.nonNull(contextPath)
	 * && !contextPath.isEmpty())) { String[] splitedUrlByContext =
	 * url.split(contextPath); ver = splitedUrlByContext[1].split("/"); } } return
	 * ver; }
	 */
