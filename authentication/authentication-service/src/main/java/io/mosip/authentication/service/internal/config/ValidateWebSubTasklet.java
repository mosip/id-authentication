package io.mosip.authentication.service.internal.config;
...
import io.mosip.authentication.service.internal.batch.CredentialStoreJobExecutionListener;
import io.mosip.authentication.service.internal.batch.MissingCredentialsItemReader;
import io.mosip.authentication.service.internal.listener.InternalAuthIdChangeEventsWebSubInitializer;
...
/**
 * Note: this class is never registered as a Spring @Configuration source - it is only
 * ever manually instantiated (new ValidateWebSubTasklet(...)) inside DataProcessingBatchConfig
 * to run as a Tasklet. Its @Bean-annotated methods below (retriggerMissingCredentials,
 * retriggerMissingCredentialsStep, asyncIdentityItemProcessor, asyncMissingCredentialRetriggerItemWriter,
 * taskExecutor) are therefore dead/unreachable duplicate code carried over unchanged for parity;
 * only the constructor and execute() are actually used.
 */
