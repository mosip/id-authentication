...
import io.mosip.authentication.service.internal.batch.CredentialStoreTasklet;
...
public class DataProcessingBatchConfig {
    ...
    /**
     * Task executor used for the batch async item processor. Named distinctly (not
     * "taskExecutor") so it cannot be picked up by Spring's @Async default-executor
     * lookup, which specifically searches for a bean named "taskExecutor" before
     * falling back to @Primary/unique Executor beans - this app already has a
     * @Primary "executor" bean (IdAuthConfig#executor) intended for that role.
     *
