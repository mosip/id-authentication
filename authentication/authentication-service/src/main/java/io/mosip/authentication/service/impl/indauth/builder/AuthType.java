...
public enum AuthType {
	...
	PI_PRI("piPri", setOf(DemoMatchType.NAME_PRI),
			...
			authReq -> Optional.of(authReq).map(AuthRequestDTO::getPii).map(PersonalIdentityDataDTO::getDemo)
					.map(DemoDTO::getPi).map(PersonalIdentityDTO::getMsPri),
			authReq -> Optional.of(authReq).map(AuthRequestDTO::getPii).map(PersonalIdentityDataDTO::getDemo)
					.map(DemoDTO::getPi).map(PersonalIdentityDTO::getMtPri)),
	 ...
	 * @param mtInfoFetcher
	 */
	AuthType(String type, Set<MatchType> associatedMatchTypes, Predicate<AuthRequestDTO> authTypeTester,
			Function<AuthRequestDTO, Optional<String>> msInfoFetcher,
			Function<AuthRequestDTO, Optional<Integer>> mtInfoFetcher) {
		this.type = type;
