...
create table kernel.sync_transaction (
		...
		status_code 	character varying(36) not null,		-- kernel.status_list.code
...
-- keys section -------------------------------------------------------------------------------
alter table kernel.sync_transaction add constraint pk_synctrn_id primary key (id)
 ;

-- indexes section -----------------------------------------------------------------------
-- create index idx_synctrn_<colX> on kernel.sync_transaction (<colX>)
-- ;

-- comments section -------------------------------------------------------------------------- 
comment on table kernel.sync_transaction is 'kernel sync_transaction table which hold all sync job details and transactions'
;
