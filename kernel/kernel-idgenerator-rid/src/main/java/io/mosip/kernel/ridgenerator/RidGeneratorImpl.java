 ...
 */
 ...
 public class RidGeneratorImpl implements RidGenerator<String> {
	 ...
	 * 
	 * @see
	 * io.mosip.kernel.core.idgenerator.RidGenerator#RidGeneration(java.
	...
	public void checkInput(String agentId, String machineId) {
		...
		if (agentId == null || machineId == null) {
			...
			throw new NullValueException(RidGeneratorConstants.MOSIP_NULL_VALUE_ERROR_CODE.getErrorCode(),
					RidGeneratorConstants.MOSIP_NULL_VALUE_ERROR_CODE.getErrorMessage());
		}
		if (agentId.isEmpty() || machineId.isEmpty()) {

			throw new EmptyInputException(RidGeneratorConstants.MOSIP_EMPTY_INPUT_ERROR_CODE.getErrorCode(),
					RidGeneratorConstants.MOSIP_EMPTY_INPUT_ERROR_CODE.getErrorMessage());
		}
		if (agentId.length() < 4 || machineId.length() < 5) {

			throw new InputLengthException(RidGeneratorConstants.MOSIP_INPUT_LENGTH_ERROR_CODE.getErrorCode(),
