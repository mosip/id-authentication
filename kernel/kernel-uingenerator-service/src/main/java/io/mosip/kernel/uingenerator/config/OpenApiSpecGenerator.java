...
/**
 * Tries to interrogate vertx router and build an OpenAPI specification. Tries
 * to interrogate handlers of route with OpenApi Operation methods and
 * cross-reference with route information. By no means all OpenApi 3 spec is
 * covered, so this part will be adjusted based on use cases encountered.
 * 
 * @author Ritesh Sinha
 * @since 1.0.0
 */
public final class OpenApiSpecGenerator {
	...
	private static  Map<String, PathItem> extractAllPaths(Router router) {
	...
	private static  void extractOperationInfo(Router router, Map<String, PathItem> paths) {
				...
				handlers.forEach(handler -> {
					try {
						...
						Arrays.stream(delegate.getDeclaredMethods()).distinct().forEach(method -> {
							io.swagger.v3.oas.annotations.Operation annotation = method
