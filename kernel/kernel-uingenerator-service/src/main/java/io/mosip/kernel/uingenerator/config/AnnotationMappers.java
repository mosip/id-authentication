...
/**
 * Simple OpenApi Annotation mapping to OpenApi models. This part needs
 * modifications to cover all spec.
 * 
 * @author Ritesh Sinha
 * @since 1.0.0
 *
...
final class AnnotationMappers {
	...
	static void decorateOperationFromAnnotation(Operation annotation, io.swagger.v3.oas.models.Operation operation) {
		...
		if (annotation.requestBody().content().length != 0) {
			...
			Content cont = new Content().addMediaType("application/json", new MediaType().schema(model));
