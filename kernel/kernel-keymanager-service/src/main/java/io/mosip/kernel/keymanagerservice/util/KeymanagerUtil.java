...
public class KeymanagerUtil {
	...
	public byte[] decryptKey(byte[] key,PrivateKey privateKey) {
		...
		final byte keySplitterFirstByte = keySplitter.getBytes()[0];
		 keyDemiliterIndex = getSplitterIndex(key, keyDemiliterIndex,
					keySplitterLength, keySplitterFirstByte);
		 byte[] encryptedKey = copyOfRange(key, 0, keyDemiliterIndex);
			byte[] encryptedData = copyOfRange(key, keyDemiliterIndex + keySplitterLength,
					cipherKeyandDataLength);
			byte[] decryptedSymmetricKey=decryptor.asymmetricPrivateDecrypt(privateKey, encryptedKey);
			SecretKey symmetricKey=new SecretKeySpec(decryptedSymmetricKey, 0,decryptedSymmetricKey.length, symmetricAlgorithmName);
