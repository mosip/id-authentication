...
public class IdRepoDaoImpl implements IdRepoDao {
	...
	int sqlStatus = jdbcTemplate.update(UIN_INSERT, uinRefId, uin, env.getProperty("mosip.idrepo.status.registered"),
		"cr_by", new Date(), "upd_by", new Date(), false, new Date());

	if (sqlStatus == 1) {
	    sqlStatus = jdbcTemplate.update(UIN_DETAIL_INSERT, uinRefId, identityInfo.toString().getBytes(), "cr_by",
	...
	Uin uinObject =
		jdbcTemplate.queryForObject(UIN_SELECT, new Object[] { uin }, new BeanPropertyRowMapper<>(Uin.class));
	...
	if (jdbcTemplate.update(UIN_DETAILS_UPDATE, identityInfo,
		jdbcTemplate
