...
public interface MachineRepository extends BaseRepository<Machine, String> {
	 ...
	 */
	@Query(value = "SELECT mm.id, mm.name, mm.mac_address, mm.serial_num, mm.ip_address, mm.mspec_id, mm.lang_code, mm.is_active, mm.cr_by, mm.cr_dtimes, mm.upd_by, mm.upd_dtimes, mm.is_deleted, mm.del_dtimes, mm.validity_end_dtimes from master.machine_master mm inner join master.reg_center_machine rcm on rcm.machine_id = mm.id where rcm.regcntr_id = (select distinct rcm1.regcntr_id from master.reg_center_machine rcm1  where rcm1.machine_id=?1) and ((mm.cr_dtimes >?2 AND mm.cr_dtimes <=?3) or (mm.upd_dtimes >?2 AND mm.upd_dtimes<=?3) or (mm.del_dtimes >?2 AND mm.del_dtimes<=?3))", nativeQuery = true)
