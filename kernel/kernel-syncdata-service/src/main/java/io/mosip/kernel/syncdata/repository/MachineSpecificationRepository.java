...
public interface MachineSpecificationRepository extends BaseRepository<MachineSpecification, String> {
	 ...
	 * @return {@link MachineSpecification}
	 */
	@Query(value = "SELECT ms.id, ms.name, ms.brand, ms.model, ms.mtyp_code, ms.min_driver_ver, ms.descr, ms.lang_code, ms.is_active, ms.cr_by, ms.cr_dtimes, ms.upd_by, ms.upd_dtimes, ms.is_deleted, ms.del_dtimes from master.machine_spec ms where	ms.id in( select distinct mm.mspec_id from master.machine_master mm inner join master.reg_center_machine rcm on	mm.id=rcm.machine_id and rcm.regcntr_id in (select distinct rcm1.regcntr_id from master.reg_center_machine rcm1 where rcm1.machine_id = ?1 )) and ((ms.cr_dtimes > ?2 and ms.cr_dtimes <=?3) or (ms.upd_dtimes >?2 and ms.upd_dtimes <=?3) or (ms.del_dtimes > ?2 and ms.del_dtimes <=?3))", nativeQuery = true)
	List<MachineSpecification> findLatestByMachineId(String machineId, LocalDateTime lastUpdated,LocalDateTime currentTimeStamp);
}
