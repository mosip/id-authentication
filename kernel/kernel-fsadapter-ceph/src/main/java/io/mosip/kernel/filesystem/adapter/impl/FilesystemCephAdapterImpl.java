...
import io.mosip.kernel.filesystem.adapter.impl.exception.CephRunTimeException;
import io.mosip.kernel.filesystem.adapter.impl.exception.PacketNotFoundException;
import io.mosip.kernel.filesystem.adapter.impl.exception.handler.ExceptionHandler;
import io.mosip.kernel.filesystem.adapter.impl.utils.ConnectionUtil;
import io.mosip.kernel.filesystem.adapter.impl.utils.PlatformErrorConstants;
...
public class FilesystemCephAdapterImpl implements FileSystemAdapter {
	...
	// } catch (AmazonS3Exception e) {
	// LOGGER.error(LOGDISPLAY, e.getStatusCode(), e.getErrorCode(),
	// e.getErrorMessage());
	// ExceptionHandler.exceptionHandler(e);
	// } catch (SdkClientException e) {
	// ExceptionHandler.exceptionHandler(e);
	 ...
	 */
	@Override
	public void unpackPacket(String enrolmentId){
		...
		zis.close();
		   ...
		   throw new CephRunTimeException(PlatformErrorMessages.KER_FAC_RUNTIME_EXCEPTION.getMessage(), e);
		}
	}
