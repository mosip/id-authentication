package io.mosip.kernel.emailnotification.smtp.util;
...
import io.mosip.kernel.emailnotification.smtp.constant.MailNotifierArgumentErrorConstants;
...
import io.mosip.kernel.emailnotification.smtp.exception.MosipErrors;
import io.mosip.kernel.emailnotification.smtp.exception.MosipMailNotifierInvalidArgumentsException;
...
public class MailNotifierUtils {
	...
	public static void validateMailArguments(String[] mailTo, String mailSubject, String mailContent) {
		Set<MosipErrors> validationErrorsList = new HashSet<>();
		if (mailTo == null || mailTo.length == Integer.parseInt(MailNotifierConstants.DIGIT_ZERO.getValue())) {
			...
			tos.forEach(to -> {
				if (to.trim().isEmpty()) {
					validationErrorsList.add(new MosipErrors(
		...
		if (!validationErrorsList.isEmpty()) {
			throw new MosipMailNotifierInvalidArgumentsException(new ArrayList<MosipErrors>(validationErrorsList));
		}
	}
}