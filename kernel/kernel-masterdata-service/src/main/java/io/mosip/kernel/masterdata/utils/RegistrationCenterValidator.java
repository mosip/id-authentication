...
public class RegistrationCenterValidator {
	...
	public String validateCenterId(RegistrationCenterPutReqAdmDto firstObj,
			RegistrationCenterPutReqAdmDto eachRecord) {
		//if (eachRecord.getId() != null && firstObj.getId() != null) {
			if (eachRecord.getId().trim().equalsIgnoreCase(firstObj.getId().trim())) {
				return firstObj.getId();
			} else {
	...
	public void validatePrimarySencodaryLangMandatoryFields(List<RegCenterPostReqPrimAdmDto> reqRegistrationCenterDto,
		...
		for (RegCenterPostReqPrimAdmDto registrationCenterDto : reqRegistrationCenterDto) {
			...
			if (registrationCenterDto.getLangCode().equalsIgnoreCase(primaryLang)) {
			...
			} else if (secondaryLangList.contains(registrationCenterDto.getLangCode())) {
				secondaryLangValidation(reqRegistrationCenterDto, registrationCenterPostResponseDto,
						validateRegistrationCenterDtos, constraintViolationedSecList, errors, registrationCenterDto,
						validator);
			} else {
				errors.add(new ServiceError(RegistrationCenterErrorCode.SECONDARY_LANGUAGE_EXCEPTION.getErrorCode(),
						String.format(RegistrationCenterErrorCode.SECONDARY_LANGUAGE_EXCEPTION.getErrorMessage(),
	...
	private void primaryLanguageValidation(List<RegCenterPostReqPrimAdmDto> validateRegistrationCenterDtos,
		...
		if (!constraintViolations.isEmpty()) {
			...
			// call method to validate zoneid, langitidue and latitude
			validateRegCenterCreateReq(registrationCenterDto, errors);
			if (!errors.isEmpty()) {
				throw new ValidationException(errors);
			}
			// add primary language Obj to list after all validation are true
	...
	public void secondaryLangValidation(List<RegCenterPostReqPrimAdmDto> reqRegistrationCenterDto,
		...
		if (!constraintViolations.isEmpty()) {
			...
			isValid(reqRegistrationCenterDto.get(0), registrationCenterDto, errors);
			validateRegCenterCreateReq(registrationCenterDto, errors);
			if (!errors.isEmpty()) {
				registrationCenterPostResponseDto.setConstraintViolationError(errors);
				constraintViolationedSecList.add(registrationCenterDto);
	...
	// validate the given ID size and null
	public String givenIdValidation(RegistrationCenterReqAdmSecDto registrationCenterReqAdmSecDtos, String uniqueId) {
		if (registrationCenterReqAdmSecDtos.getId() != null && !registrationCenterReqAdmSecDtos.getId().isEmpty()) {
			if ((registrationCenterReqAdmSecDtos.getId().length() >= ONE)
					&& (registrationCenterReqAdmSecDtos.getId().length() <= TEN)) {

				uniqueId = registrationCenterReqAdmSecDtos.getId();
			} else {
				throw new RequestException(RegistrationCenterErrorCode.ID_SIZE.getErrorCode(),
						String.format(RegistrationCenterErrorCode.ID_SIZE.getErrorMessage(),
								registrationCenterReqAdmSecDtos.getId()));
			}
		}
		return uniqueId;
	}
	...
	public void validatePutRequest(List<RegistrationCenterPutReqAdmDto> registrationCenterPutReqAdmDto,
			...
			List<String> idLangList, List<String> langList, List<ServiceError> errors, String uniqueId) {
		for (RegistrationCenterPutReqAdmDto regCenterDto : registrationCenterPutReqAdmDto) {
			uniqueId = validateCenterId(registrationCenterPutReqAdmDto.get(0), regCenterDto);
