...
public class IntegrationTest {
	...
	public void getUinSuccessTest(TestContext context) {
		...
		client.get(port, "localhost", "/v1/vidgenerator/vid").send(ar -> {
			if (ar.succeeded()) {
				...
				try {
					...
					VidFetchResponseDto dto = objectMapper.convertValue(uinResp.getResponse(), VidFetchResponseDto.class);
	...
	public void getUinSuccessViDExpiryEmptyTest(TestContext context) {
		...
		Async async = context.async();
		WebClient client = WebClient.create(vertx);
		client.get(port, "localhost", "/v1/vidgenerator/vid?videxpiry=").send(ar -> {
			if (ar.succeeded()) {
				HttpResponse<Buffer> httpResponse = ar.result();
				LOGGER.info(httpResponse.bodyAsString());
				context.assertEquals(200, httpResponse.statusCode());
	...
	public void getUinSuccessViDExpiryInvalidPatternTest(TestContext context) {
		...
		Async async = context.async();
		WebClient client = WebClient.create(vertx);
		client.get(port, "localhost", "/v1/vidgenerator/vid?videxpiry="+(DateUtils.getCurrentDateTimeString().replace("Z", ""))).send(ar -> {
	...
	public void getUinSuccessViDExpiryInvalidExpiryDateTest(TestContext context) {
		...
		Async async = context.async();
		WebClient client = WebClient.create(vertx);
		client.get(port, "localhost", "/v1/vidgenerator/vid?videxpiry=2018-12-10T06:12:52.994Z").send(ar -> {
