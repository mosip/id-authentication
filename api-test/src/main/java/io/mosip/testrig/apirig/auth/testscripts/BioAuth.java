...
public class BioAuth extends IdAuthenticationUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO) throws AuthenticationTestException, AdminTestException, SecurityXSSException {
		...
		// Lets YAML override "env" instead of it always being forced to "Staging".
		String invalidEnv = null;
		if (request.has("invalidEnv")) {
			invalidEnv = request.getString("invalidEnv");
			request.remove("invalidEnv");
		}
		// Substitutes a real (server-issued) public JWK for a "$WALLETPUBLICJWK:<name>$"
		// token at identityKeyBinding.publicKeyJWK, if present - no-op otherwise.
		IdAuthenticationUtil.resolveWalletPublicJwk(request);

		identityRequest = buildIdentityRequest(identityRequest);

		// keyBindedTokens[].token needs a freshly-signed JWT (server enforces a short
		// iat window), so resolve it here on the raw pre-encryption identityRequest,
		// using the individualId already resolved to its real value below.
		if (identityRequest.contains("$WLAJWT:")) {
			String resolvedIndividualId = replaceIdWithAutogeneratedId(request.optString("individualId", ""), "$ID:");
			identityRequest = IdAuthenticationUtil.resolveWlaJwt(identityRequest, resolvedIndividualId);
		}
