...
export class DashBoardComponent implements OnInit {
    ...
    this.dataStorageService.getUsers(this.loginId).subscribe(
      (applicants: Applicant[]) => {
        if (applicants[appConstants.RESPONSE] !== null) {
          ...
          for (
            let index = 0;
            index < applicants[appConstants.RESPONSE].length;
            index++
          ) {
            ...
            const applicant: Applicant = {
              applicationID:
                applicants[appConstants.RESPONSE][index][
                  appConstants.DASHBOARD_RESPONSE_KEYS.applicant.preId
                ],
              name:
                applicants[appConstants.RESPONSE][index][
                  appConstants.DASHBOARD_RESPONSE_KEYS.applicant.fullname
                ],
      ...
      if (selectedOption && Number(selectedOption) === 1) {
        ...
        dialogRef.afterClosed().subscribe(confirm => {
          if (confirm) {
            ...
            this.dataStorageService
              .cancelAppointment(new BookingModelRequest(element.regDto))
              .subscribe(
                response => {
                  const message = {
                    case: "MESSAGE",
  ...
  onModifyMultipleAppointment() {
    ...
    this.router.navigate(
      ["../../", "pre-registration", this.loginId, "pick-center"],
  ...
  private createIdentityJSON(identityModal: IdentityModel) {
    const identity = new IdentityModel(
      ...
      [
        new AttributeModel(
          identityModal.city[0].language,
          identityModal.city[0].value
        ),
        new AttributeModel(
          identityModal.city[1].language,
          identityModal.city[1].value
        )
      ],
      [
        new AttributeModel(
  ...
  private createRequestJSON(requestModal: RequestModel) {
    const identity = this.createIdentityJSON(
      requestModal.demographicDetails.identity
    );
