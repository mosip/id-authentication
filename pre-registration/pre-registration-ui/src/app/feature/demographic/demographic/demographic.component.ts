...
export class DemographicComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.dataStorageService
      .getSecondaryLanguageLabels(this.secondaryLang)
      .subscribe(response => {
        this.secondaryLanguagelabels = response["demographic"];
      });

    await this.getSecondaryLabels();
    console.log(
      this.secondaryLanguagelabels,
      this.secondaryLanguagelabels.case
    );

    if (!this.dataModification) this.consentDeclaration();
  }

  private getSecondaryLabels() {
  ...
  private setFormControlValues() {
    if (!this.dataModification) {
      ...
      this.formControlValues = {
        fullName: this.user.request.demographicDetails.identity.fullName[0]
          .value,
  ...
  //       this.residenceStatus =
  //         response[appConstants.DEMOGRAPHIC_RESPONSE_KEYS.residentTypes];
  //       resolve(true);
  //     });
  //   });
  // }
