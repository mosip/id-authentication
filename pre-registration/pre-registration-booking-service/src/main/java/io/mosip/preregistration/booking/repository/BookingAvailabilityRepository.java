...
public interface BookingAvailabilityRepository extends BaseRepository<AvailibityEntity, String> {
	public static final String distDate = "SELECT DISTINCT availability_date  FROM prereg.reg_available_slot where regcntr_id=:regcntrId and availability_date>=:fromDate and availability_date<=:toDate order by availability_date ASC";
...
//	@Query(value = findall, nativeQuery = true)
	...
	public List<java.sql.Date> findDate(@Param("regcntrId") String regcntrId,@Param("fromDate") LocalDate fromDate, @Param("toDate") LocalDate toDate);
