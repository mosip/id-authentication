...
public class DeviceMappingController extends BaseController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			auditFactory.audit(AuditEvent.GET_ONBOARDING_DEVICES_TYPES, Components.DEVICE_ONBOARD,
					"Get the types of onboarding devices", SessionContext.getSessionContext().getUserContext().getUserId(),
					RegistrationConstants.ONBOARD_DEVICES_REF_ID_TYPE);

			// Set Machine ID
			SessionContext.getSessionContext().getMapObject().put(RegistrationConstants.MACHINE_ID, "1947");
	...
	private void displayDevices() {
		try {
			...
			Map<String, List<DeviceDTO>> devicesMap = mapMachineService.getDeviceMappingList(
					SessionContext.getSessionContext().getUserContext().getRegistrationCenterDetailDTO()
							.getRegistrationCenterId(),
					(String) SessionContext.getSessionContext().getMapObject().get(RegistrationConstants.MACHINE_ID));
			...
			SessionContext.getSessionContext().getMapObject().put(RegistrationConstants.ONBOARD_DEVICES_MAP, devicesMap);
			...
			SessionContext.getSessionContext().getMapObject().put(RegistrationConstants.ONBOARD_DEVICES_MAP_UPDATED,
					upadtedDevicesMap);
		} catch (RuntimeException runtimeException) {
			LOGGER.error(DEVICE_ONBOARD_CONTROLLER, APPLICATION_NAME, APPLICATION_ID,
	...
	private void mapAvailableDevices(MouseEvent mouseEvent) {
		...
		try {
			...
			Map<String, Set<DeviceDTO>> updatedDevicesMap = (Map<String, Set<DeviceDTO>>) SessionContext.getSessionContext().getMapObject().get(RegistrationConstants.ONBOARD_DEVICES_MAP_UPDATED);
	...
	private void submit(ActionEvent actionEvent) {
		...
		try {
			auditFactory.audit(AuditEvent.UPDATE_DEVICES_ONBOARDING, Components.DEVICE_ONBOARD,
					"Updating mapping of devices", SessionContext.getSessionContext().getUserContext().getUserId(),
					RegistrationConstants.ONBOARD_DEVICES_REF_ID_TYPE);

			// Get updated added and removed devices
			Map<String, Set<DeviceDTO>> devicesMap = (Map<String, Set<DeviceDTO>>) SessionContext.getSessionContext().getMapObject()
			...
			devicesMap = (Map<String, Set<DeviceDTO>>) SessionContext.getSessionContext().getMapObject()
			...
			// Get the Machine ID
			String machineId = (String) SessionContext.getSessionContext().getMapObject()
	...
	private Map<String, List<DeviceDTO>> getDevicesByType(String deviceType) {
		...
		try {
			...
			actualDevicesMap = new HashMap<>((Map<String, List<DeviceDTO>>) SessionContext.getSessionContext().getMapObject()
					.get(RegistrationConstants.ONBOARD_DEVICES_MAP));
