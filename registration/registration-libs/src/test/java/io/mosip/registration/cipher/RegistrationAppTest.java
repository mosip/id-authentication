...
public class RegistrationAppTest {
	...
	@Test
	public void hasUpdateTrueTest() throws Exception {
		...
		PowerMockito.whenNew(Manifest.class).withArguments(fileInputStream).thenReturn(localmanifest);

		PowerMockito.whenNew(Manifest.class).withArguments(urlInputStream).thenReturn(servermanifest);

		Map<String, Attributes> localAttributes = new HashMap<>();
		Map<String, Attributes> serverAttributes = new HashMap<>();
