### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: IDA DB Artifacts deploy
### -- Deploy Module 	: MOSIP IDA
### -- Purpose    		: To deploy MOSIP IDA Database DB Artifacts.       
...
LOG="${LOG_PATH}mosip_ida-${today}.log"
touch $LOG
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('idauser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating ida database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Ida user already exist" | tee -a $LOG 2>&1
fi
