...
public class CreateVid extends IdRepoTestsUtil implements ITest {
	...
	public void CreateVidTest(TestParameters objTestParameters, String testScenario,
		...
		//{
			...
			logger.info("************* IdRepo VID request ******************");
			Reporter.log("<b><u>VID create request</u></b>");
			//Assert.assertEquals(modifyRequest(testCaseName.listFiles(), tempMap, mapping, "create"), true);
		//}
		logger.info("******Post request Json to EndPointUrl: " + IdRepoUtil.getCreateVidPath() + " *******");
		postRequestAndGenerateOuputFileForUINGeneration(testCaseName.listFiles(), IdRepoUtil.getCreateVidPath(),
