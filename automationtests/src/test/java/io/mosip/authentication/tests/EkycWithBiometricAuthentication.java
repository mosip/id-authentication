...
public class EkycWithBiometricAuthentication extends IdaScriptsUtil implements ITest {
	...
	/**
	 * Test method for ekyc with biometric authentication execution
	 * 
	...
	public void idaEkycWithBiometricAuthenticationTest(TestParameters objTestParameters, String testScenario,
			String testcaseName) {
		...
		Assert.assertEquals(postRequestAndGenerateOuputFile(testCaseName.listFiles(),
				RunConfig.getEndPointUrl() + RunConfig.getEkycPath() + extUrl, "request", "output-1-actual-res", 200),
				true);
