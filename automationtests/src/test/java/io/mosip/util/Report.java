...
// ~ Methods --------------------------------------------------------------
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, "Reports.html"))));
	}
	...
	protected void generateMethodSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				String testName = testContext.getAttribute("tcName").toString();
				System.out.println("tc name is "+testName);
	...
	protected void generateMethodDetailReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				ITestContext testContext = r2.getTestContext();
				if (r.values().size() > 0) {
					m_out.println("<h1>" + testContext.getAttribute("tcName").toString() + "</h1>");
				}
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("<td><a href=\"#m" + m_methodIndex + "\">" + qualifiedName(method, TC) + " "
	...
	private String qualifiedName(ITestNGMethod method,String tC) {
