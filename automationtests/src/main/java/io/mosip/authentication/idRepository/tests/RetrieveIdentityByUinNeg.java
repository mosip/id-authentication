...
public class RetrieveIdentityByUinNeg extends IdRepoTestsUtil implements ITest {
	...
	public Object[][] getTestCaseList() {
		...
		return DataProviderClass.getDataProvider(
				RunConfigUtil.getResourcePath()
						+ RunConfigUtil.objRunConfig.getScenarioPath(),
	...
	public void setResultTestName(ITestResult result) {
		try {
			...
			f.set(baseTestMethod, RetrieveIdentityByUinNeg.testCaseName);
		} catch (Exception e) {
	...
	public void RetrieveUIN(TestParameters objTestParameters, String testScenario, String testcaseName) {
		...
		//String uinNumber = JsonPrecondtion.getValueFromJson(expIdentity, "UIN");
		String uinNumber=JsonPrecondtion.getValueFromJson(getContentFromFile(testCaseName.listFiles(), "uin"), "uin");
		logger.info("******Post request Json to EndPointUrl: " + IdRepoUtil.getRetrieveIdentityByUIN(uinNumber)
				+ " *******");
		
		//logger.info("******Post request Json to EndPointUrl: " + IdRepoUtil.getRetrieveUINByVIDPath(vidNumber) + " *******");
		getRequestAndGenerateOuputFileWithResponse(testCaseName.getPath(),IdRepoUtil.getRetrieveIdentityByUIN(uinNumber),"output-1-actual-res",AUTHORIZATHION_COOKIENAME,cookieValue);
