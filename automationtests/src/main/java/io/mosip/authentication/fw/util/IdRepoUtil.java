...
public class IdRepoUtil extends AuthTestsUtil {
		...
		try {
			...
			if (retrieveIdRepo(uinNumber)) {
				String value = XmlPrecondtion.getValueFromXmlUsingMapping(
						Paths.get(new File(RunConfigUtil.getResourcePath()
								+ RunConfigUtil.objRunConfig.getStoreUINDataPath() + "/" + uinNumber + ".json")
										.getAbsolutePath())
								.toString(),
						new File(RunConfigUtil.getResourcePath()
	...
	public static String getRetrieveIdentityByUIN(String uinNumer) {
		String url = RunConfigUtil.objRunConfig.getIdRepoEndPointUrl() + RunConfigUtil.objRunConfig.getIdRepoRetrieveDataPath();
