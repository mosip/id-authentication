...
//	@Autowired
	...
	public <T> T requestSync(@Valid RestRequestDTO request) throws RestServiceException {
		...
		try {
			...
			mosipLogger.debug(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_REQUEST_SYNC,
					"Request received at : " + requestTime);
			mosipLogger.debug(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_REQUEST_SYNC, PREFIX_REQUEST + request);
			...
			mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_REQUEST_SYNC,
			...
			if (e.getCause() != null && e.getCause().getClass().equals(TimeoutException.class)) {
				...
				mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER, REQUEST_SYNC_RUNTIME_EXCEPTION,
	...
	public Supplier<Object> requestAsync(@Valid RestRequestDTO request) {
		mosipLogger.debug(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_REQUEST_ASYNC, PREFIX_REQUEST + request);
		Mono<?> sendRequest = request(request);
		sendRequest.subscribe();
		mosipLogger.debug(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_REQUEST_ASYNC, "Request subscribed");
	...
	private RestServiceException handleStatusError(WebClientResponseException e, Class<?> responseType) {
		try {
			mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_HANDLE_STATUS_ERROR,
					"Status error : " + e.getRawStatusCode() + " " + e.getStatusCode() + "  " + e.getStatusText());
			if (e.getStatusCode().is4xxClientError()) {
				if (e.getRawStatusCode() == 401 || e.getRawStatusCode() == 403) {
					mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER,
					...
					mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER, "Throwing AuthenticationException",
							errorList.toString());
			...
			mosipLogger.error(IdRepoLogger.getId(), CLASS_REST_HELPER, METHOD_HANDLE_STATUS_ERROR, ex.getMessage());
