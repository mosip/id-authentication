...
public class UMCValidator {
	...
	public boolean isValidUMC(String registrationId) throws ApisResourceAccessException {
		...
		boolean umc=false;
		if(validateRegistrationCenter(rcmDto.getRegcntrId(),primaryLanguagecode,rcmDto.getPacketCreationDate().toString())
			&& validateMachine(rcmDto.getMachineId(),primaryLanguagecode,rcmDto.getPacketCreationDate().toString()) 
			&& validateUMCmapping(rcmDto.getPacketCreationDate().toString(),rcmDto.getRegcntrId(),rcmDto.getMachineId(),
