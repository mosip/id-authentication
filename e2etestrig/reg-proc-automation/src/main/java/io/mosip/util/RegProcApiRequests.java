...
public class RegProcApiRequests extends BaseTest {
	...
	public String getUinFromIDRepo(String regId,String idRepoToken) {
			String idRepoUrl="/idrepository/v1/identity/rid/";
			Cookie.Builder builder = new Cookie.Builder("Authorization",idRepoToken);
			
	Response idRepoResponse = given().cookie(builder.build()).relaxedHTTPSValidation().when().get(ApplnURI+idRepoUrl+regId).then().extract().response();
