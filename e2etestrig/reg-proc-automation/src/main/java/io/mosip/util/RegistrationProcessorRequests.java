...
public class RegistrationProcessorRequests extends BaseTest {
	...
	public boolean UploadPacket(File packet) throws ParseException, FileNotFoundException, IOException {
		...
		try {
		Assert.assertTrue(actualResponse.jsonPath().get("id").equals("mosip.registration.packet"));
		Assert.assertTrue(actualResponse.jsonPath().get("version").equals("1.0"));
...
public String getUin(String regId) {
	String idRepoToken=getToken("syncTokenGenerationFilePath");
		...
		String uin=apiRequests.getUinFromIDRepo(regId, idRepoToken);
		return uin;
}
public static void main(String[] args) {
	RegistrationProcessorRequests req=new RegistrationProcessorRequests();
	req.getUin("10006100360000220190702093517");
}
}
